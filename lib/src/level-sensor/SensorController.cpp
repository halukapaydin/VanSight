...
bool SensorController::addSensor(int index, int pin, float minResistance, float maxResistance, float referenceResistor, float adcResolution, float supplyVoltage) {
    if (index >= _count) return false;
    
    // Create new LevelSensor object
    _sensors[index] = new LevelSensor(pin, minResistance, maxResistance, referenceResistor, adcResolution, supplyVoltage);
    return true;
}
